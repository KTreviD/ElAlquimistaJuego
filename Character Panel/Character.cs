...
public class Character : MonoBehaviour
{
	...
	private void Drop(BaseItemSlot dropItemSlot)
	{
		...
		if(dragItemSlot.Amount >= 1 && dropItemSlot is EquipmentSlot)
		{
			if(dropItemSlot.CanAddStack(dragItemSlot.Item))
			{
				return;
			}
			if(dragItemSlot.Item is EquippableItem)
			{
				Equip((EquippableItem)dragItemSlot.Item);
			}
		}
		else if (dropItemSlot.CanAddStack(dragItemSlot.Item))
		{
			...
			{
				Unequip((EquippableItem)dragItemSlot.Item);
			}
			AddStacks(dropItemSlot);
		}
		else if (dropItemSlot.CanReceiveItem(dragItemSlot.Item) && dragItemSlot.CanReceiveItem(dropItemSlot.Item))
		{
			if(dropItemSlot.Amount > 1)
			{
				if(dropItemSlot is EquipmentSlot || dragItemSlot is EquipmentSlot)
				{
					Equip((EquippableItem)dropItemSlot.Item);
				}
				else
				{
